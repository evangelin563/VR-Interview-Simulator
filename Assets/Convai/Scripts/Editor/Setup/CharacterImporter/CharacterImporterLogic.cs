    ...
    public class CharacterImporterLogic
    {
        ...
        public async Task DownloadCharacter(string characterID)
        {
            ...
            try
            {
                ...
                string fullPath = $"{DirectoryUtility.DefaultAvatarFolder}/{args.Avatar.name}"; // Combine folder and name
                string projectPath = "Assets/" + fullPath; // Add "Assets/" to make it a project path
                string relativePath = projectPath.Substring(projectPath.IndexOf("Assets/") + "Assets/".Length); // Get relative to "Assets/"


                GameObject avatar = PrefabHelper.CreateAvatarPrefab(args.Metadata, projectPath, avatarConfig: avatarLoaderSettings.AvatarConfig); // Use project path for saving

                // Refresh the Asset Database to make sure the new prefab is visible in the project.
